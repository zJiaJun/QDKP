...
function myClass.Refresh(self)
    ...
    elseif not Session then
      QDKP2frame1_CurrentSession:SetText("没有正在进行的活动。")
    else
      local _,Name=QDKP2_GetSessionInfo(Session)
      QDKP2frame1_CurrentSession:SetText("当前活动："..QDKP2_COLOR_WHITE..tostring(Name))
    end
    ...
    if QDKP2backup.DATE then
      TimeString = "最近备份： "..QDKP2_GetDateTextFromTS(QDKP2backup.DATE)
    else
      TimeString = "没有发现备份文件。"
    end
