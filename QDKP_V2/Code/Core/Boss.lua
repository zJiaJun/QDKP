...
function QDKP2_GetBossAward(boss,zone)
  ...
  local DKPType, award
  local instDiff=GetInstanceDifficulty()  --1: 10-normal, 2:25-normal, 10-hero, 25-hero
  zone=zone or GetRealZoneText()
  local zoneEng=QDKP2zoneEnglish[zone] or zone
  if zoneEng == "Trial of the Crusader" and (instDiff==3 or instDiff==4) then
    zone="Trial of the Grand Crusader"
    zoneEng="Trial of the Grand Crusader"
  end
  if zoneEng == "Icecrown Citadel" and (instDiff==3 or instDiff==4) then
    zone="Heroic Icecrown Citadel"
    zoneEng="Heroic Icecrown Citadel"
  end
  zone=string.lower(zone)
  zoneEng=string.lower(zoneEng)
  if     instDiff==1 or instDiff==3 then DKPType="DKP_10"
  elseif instDiff==2 or instDiff==4 then DKPType="DKP_25"
  end
